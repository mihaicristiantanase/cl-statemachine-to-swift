    ...
    (dolist (action (slot-value *machine* 'actions))
      (let ((ap (sym->pascalcase action))
        ...
        (define-swift-fun (format nil "doAction~a" ap) "_ completion: @escaping Completion"
          ...
          (wl (format nil "lastAction = .~a" ac))
          (define-swift-block "do"
              (wl (format nil "let transition = try findTransition(.~a)" ac))
            (define-swift-block (format nil "if action~a == nil" ap)
                (wl (format nil "completion(false, Err.transitionNotSet(state, .~a))" ac))
              (wl "return")
              )
